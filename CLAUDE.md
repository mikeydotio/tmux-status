...
- **`scripts/tmux-status-poke`** (Bash) — Wakes the daemon for one immediate tick by sending `SIGUSR1` only to the pid in `renderd.lock`, after its command line verifies it is `tmux-status-renderd`. A missing, stale, or unrelated pid is a no-op; there is no process-name broadcast because that can kill a daemon in its startup window. Invoked ONLY on infrequent events — tmux `set-hook` for `after-new-window`/`after-split-window`/`session-created`/`client-session-changed` (in `status.conf`, run with `run-shell -b`), and the context hook after it writes the bridge file. This closes the cold-start gap (blank Claude lines on a fresh or `/clear`'d session) without adding any fork to the per-render path. Always exits 0.
...
**Authentication contract:** the collector observes the user's own sessions, so it must run as they run — **piggybacking the auth already present, never authenticating itself**. The pane command uses `/usr/bin/env -u` to remove alternate-*provider* selectors (`CLAUDE_CODE_USE_BEDROCK`/`VERTEX`/`FOUNDRY`/`MANTLE`/`ANTHROPIC_AWS`) and third-party credentials (`ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`) after the pane shell has sourced startup files; scrubbing only the daemon environment is insufficient because `.zshenv` can re-export them.

What is *not* scrubbed matters as much: `CLAUDE_CONFIG_DIR`, `ANTHROPIC_BASE_URL` and **`CLAUDE_CODE_OAUTH_TOKEN`** all select or carry the user's own login rather than redirecting to another provider. `CLAUDE_CODE_OAUTH_TOKEN` was scrubbed until TS-57: it is Claude Code's own first-party token (`claude setup-token`), and on a machine where it is what real sessions authenticate with — the CLI's `/usage` → Status tab names it under `Auth token:` — removing it left the pane with no credential at all. That produced `cli_not_authenticated` on every poll for four days while the user's own CLI was plainly logged in. `tests/../test_cli_usage.py` pins both halves: the first-party token is preserved, the provider selectors are still removed.

`--usage-inherit-auth-env` is the explicit opt-out from all scrubbing. A settings-file `apiKeyHelper` cannot be removed through the environment, so an opened Usage dialog without subscription windows must report `usage_no_limit_windows` — which is also the honest result when the inherited identity simply has no subscription rate-limit windows to report.
