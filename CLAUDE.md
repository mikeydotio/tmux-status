...
The fork-free refactor moved heavy work off the render path, but tmux still spawns each `#()` reader by `fork()`+`socketpair()` **inside the tmux server process** — so every status line costs one *server* fd per redraw. The server's fd budget is shared with one fd per attached client, and macOS's launchd default soft `RLIMIT_NOFILE` is only **256**. Long-lived clients (abandoned mosh/SSH reconnects) accumulate and pin that budget until a redraw can't get a socketpair, and tmux prints `<'…' didn't start>` for the `#()` lines (RCA: `.rca/tmux-status-didnt-start-fd-exhaustion/`). Implications when changing the render path: keep the per-pane `#()` job count low (do **not** add status lines casually), `tmux-status-session` raises `ulimit -n` before creating a server, `install.sh` warns when a running server's soft limit is low, and `tmux-status-prune-clients` reclaims fds by detaching idle clients. A pane shell's `ulimit -n` may be `.zshrc`-inflated — read the server's real limit via `tmux run-shell 'ulimit -Sn'`.
